...
class TestAddInHead(unittest.TestCase):
    def test_main(self):
        ...
        self.assertTrue(all([s_list.head is nodes[2], s_list.tail is nodes[2]]))

        # case 2
        s_list.add_in_head(nodes[1])
        self.assertEqual(s_list.get_all_nodes(), nodes[1:]) # [2->] ==> [1->2->]
        self.assertTrue(all([s_list.head is nodes[1], s_list.tail is nodes[2]]))
        s_list.add_in_head(nodes[0])
        self.assertEqual(s_list.get_all_nodes(), nodes) # [1->2->] ==> [0->1->2->] 
        self.assertTrue(all([s_list.head is nodes[0], s_list.tail is nodes[2]]))
